...
#define SIGN(n) (n <= 0?( n < 0 ?-1:0):1) //add by wangcaijie
...
bool BZPlannerROS::setPlan(const std::vector<geometry_msgs::PoseStamped>& plan)
{
    ...
    if (planner_util_.setPlan(plan))
    {
        ...
        tf::poseStampedTFToMsg(current_pose_tf_, start_pose);
        try
        {
...
bool BZPlannerROS::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
    ...
    geometry_msgs::PoseStamped current_pose, global_current_pose;
    tf::Stamped<tf::Pose> t1,t2;
    if (!costmap_ros_->getRobotPose(current_pose_tf_))
    {
        ROS_ERROR("Could not get robot current_pose tf");
        return false;
    }

    // if(!planner_util_.getLocalPlan(current_pose_tf_, transformed_plan)) //transform goal based on local plan
    // {
    //     ROS_ERROR("Could not transform local plan");
    //     return false;
    // }

    tf::poseStampedTFToMsg(current_pose_tf_, current_pose);
    ...
    if(motion_status_ == NAVIGATION)
    {
        ...
        // {
        //     if(!globalPlanConversion(goal_pose_, transformed_plan, select)){
    ...
    tf::Pose goal_pose_tf,global_current_pose_tf;
    tf::poseMsgToTF(global_current_pose.pose, global_current_pose_tf);
    ...
    publishBezierPlan(bz_ctrl_points);//"global_plan"
    publishBezierCtrlPoints(bz_ctrl_points);//"ctrl_points"
                                   ...
                                   base_local_planner::Trajectory& traj)
{
    ...
    init_pose[0] = current_pose_tf_.getOrigin().getX();
    init_pose[1] = current_pose_tf_.getOrigin().getY();
    init_pose[2] = tf::getYaw(current_pose_tf_.getRotation());
    // simulation vel
...
bool BZPlannerROS::goalReachXY()
{
    ...
    if (fabs(current_pose_tf_.getOrigin().getX() - goal_x) <= x_tolerance_
        && fabs(current_pose_tf_.getOrigin().getY() - goal_y) <= y_tolerance_)
    {
        return true;
    }
    return false;
...
bool BZPlannerROS::goalReachEucDis()
{
    ...
    double goal_x = goal_pose.getOrigin().getX();
    double goal_y = goal_pose.getOrigin().getY();
    if (base_local_planner::getGoalPositionDistance(current_pose_tf_, goal_x, goal_y) <= xy_tolerance_)
    {
        return true;
    }
    return false;
...
bool BZPlannerROS::goalReachXYYaw()
{
    ...
    double goal_th = tf::getYaw(goal_pose.getRotation());
    if (goalReachXY()
        && base_local_planner::getGoalOrientationAngleDifference(current_pose_tf_, goal_th) <= yaw_tolerance_)
    {
        return true;
    }
    return false;
